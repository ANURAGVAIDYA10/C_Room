...
import { forceLogout } from "../../../utils/forceLogout"; // Import force logout utility
...
export default function UserDropdown({ isOpen, onToggle }: UserDropdownProps) {
    ...
    const loadUserData = async () => {
      if (currentUser?.uid) {
        try {
          ...
          // In a real implementation, fetch user data from backend to get avatar
  ...
  const handleSignOut = async () => {
    ...
    try {
      ...
      // Step 2: Clear backend JWT session (this is what actually logs you out)
      console.log("2. Clearing backend JWT session...");
      try {
        await authApi.logout();
        console.log("✅ Backend JWT session cleared");
      } catch (backendError) {
        console.warn("⚠️ Backend logout error (continuing):", backendError);
      }
      
      // Step 3: Minimal Firebase cleanup (optional, since we don't rely on it)
      console.log("3. Cleaning up Firebase auth...");
      try {
        await signOut(auth);
        console.log("✅ Firebase auth cleaned up");
      } catch (firebaseError) {
        console.warn("⚠️ Firebase cleanup error (continuing):", firebaseError);
      }
      
      // Step 4: Final security cleanup
      console.log("4. Final security cleanup...");
      localStorage.clear();
      sessionStorage.clear();
      console.log("✅ Security cleanup completed");
      
      // Step 5: Redirect to sign in
      console.log("5. Redirecting to sign in page...");
      navigate("/signin", { replace: true });
      console.log("✅ Redirect initiated");
      
      // Step 6: Hard redirect backup
      setTimeout(() => {
        console.log("6. Hard redirect backup...");
      ...
      // Emergency redirect
      console.log("EMERGENCY: Emergency redirect...");
      localStorage.clear();
      sessionStorage.clear();
      window.location.href = "/signin";
    } finally {
      console.log("7. Closing dropdown...");
      closeDropdown();
  ...
  return (
    <div className="relative">
      ...
      <Dropdown
        ...
        className="absolute z-40 left-full top-0 mt-0 w-[260px] flex flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark"
      >
        <div>
        ...
        <button
          ...
          className="flex items-center gap-3 px-3 py-2 mt-3 font-medium text-gray-700 rounded-lg group text-theme-sm hover:bg-indigo-200 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300 w-full text-left"
        >
          <svg
        ...
        {/* DEBUG: Force logout button - REMOVE AFTER TESTING */}
        <button
          onClick={async (e) => {
            e.preventDefault();
            e.stopPropagation();
          ...
          className="flex items-center gap-3 px-3 py-2 mt-2 font-medium text-red-600 rounded-lg group text-theme-sm bg-red-50 hover:bg-red-100 w-full text-left border border-red-200"
        >
          <svg className="fill-red-500" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
