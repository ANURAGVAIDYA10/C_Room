...
export default function CompleteInvitation() {
  ...
  useEffect(() => {
    const verifyInvitation = async () => {
      try {
        ...
        if (!emailParam) {
          throw new Error("Invalid invitation link - email parameter missing.");
        }

        // Verify the invitation exists in our system
        const response = await invitationApi.verifyInvitationByEmail(emailParam);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (result.user.email !== invitationData?.email) {
        setMessage({
          type: "error", 
          text: `The Google account email (${result.user.email}) does not match the invited email (${invitationData?.email}). Please sign in with the correct Google account.`
        });
        setLoading(false);
        return;
      }
      ...
      // Use the display name from Google/Microsoft, fallback to email if not available
      ...
      if (firebaseError.code === 'auth/email-already-in-use') {
        ...
        try {
          // Use the email from invitation data since result is not available in catch block
          const fullName = invitationData?.email || "";
          await completeInvitation(fullName);
  ...
  const handleMicrosoftSignIn = async () => {
    ...
    try {
      ...
      if (result.user.email !== invitationData?.email) {
        setMessage({
          type: "error", 
          text: `The Microsoft account email (${result.user.email}) does not match the invited email (${invitationData?.email}). Please sign in with the correct Microsoft account.`
        });
        setLoading(false);
        return;
      }
      ...
      // After successful sign-in, complete the invitation
      // Use the display name from Microsoft, fallback to email if not available
      const fullName = result.user.displayName || result.user.email?.split('@')[0] || "";
      await completeInvitation(fullName);
    } catch (error) {
      ...
      if (firebaseError.code === 'auth/email-already-in-use') {
        ...
        try {
          // Use the email from invitation data since result is not available in catch block
          const fullName = invitationData?.email?.split('@')[0] || "";
          await completeInvitation(fullName);
        } catch (completionError) {
          console.error('Error completing invitation:', completionError);
          setMessage({
  ...
  const completeInvitation = async (fullName: string) => {
    ...
    try {
      // Complete the invitation with user details
      const completionData = {
        email: invitationData.email,
        fullName: fullName,
        role: invitationData.role,
        departmentId: invitationData.departmentId || null,
        organizationId: invitationData.organizationId || undefined
      };
       
      // Use the existing completeInvitation endpoint but with OAuth-specific data
      // Include role, department, and organization info
      const response = await invitationApi.completeInvitation({
        token: "", // Token not needed for this flow
        email: completionData.email,
        fullName: completionData.fullName,
        password: "oauth_account_placeholder", // Use a placeholder for OAuth accounts
        role: completionData.role,
        departmentId: completionData.departmentId,
        organizationId: completionData.organizationId
      });
          
      if (response.message) {
        setMessage({type: "success", text: response.message || "Registration completed successfully! Redirecting to dashboard..."});
      ...
      // Check if it's the specific error about user already existing
      const errorMessage = (error as Error).message;
      if (errorMessage.includes("User already exists in Firebase")) {
        // If user already exists, that's fine - they can still complete the invitation
        // This might happen when the user was created via OAuth but invitation wasn't completed
        setMessage({type: "success", text: "Registration completed successfully! Redirecting to dashboard..."});
        // Redirect to dashboard after a short delay
        setTimeout(() => {
          navigate("/ecommerce/dashboard");
        }, 2000);
